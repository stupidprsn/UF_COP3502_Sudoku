...
class GameScene(Scene):
    ...
    def __init__(self, screen: pygame.Surface, textManager: TextManager, difficulty: Difficulty):
        ...
        self._buttons: list[Button] = [
            Button(textManager, screen, "RESET", (config.WINDOW_WIDTH // 2) - config.HOME_BUTTON_OFFSET,
                   config.HOME_BUTTON_Y, self.resetButton),
            Button(textManager, screen, "RESTART", (config.WINDOW_WIDTH // 2), config.HOME_BUTTON_Y, self.restartButton),
            Button(textManager, screen, "EXIT", (config.WINDOW_WIDTH // 2) + config.HOME_BUTTON_OFFSET,
    ...
    def update(self, events: list[pygame.event.Event]) -> None:
        for event in events:
            if event.type == pygame.MOUSEBUTTONUP:
                pos: Optional[tuple[int, int]] = self._board.click(event.pos[0], event.pos[1])
                if pos is not None:
                ...
                if event.key == pygame.K_BACKSPACE or event.key == pygame.K_DELETE:
                    ...
                    if self._board.is_full():
                        if self._board.check_board():
                            self._nextScene = ResultScene(self._screen, self._textManager, True)
                        else:
                            self._nextScene = ResultScene(self._screen, self._textManager, False)
                        pass
